...
/** One row per role in the {@code held_prompts} table, each change its own transaction. */
@Singleton
class SqliteHeldPrompts implements HeldPrompts {
    ...
    SqliteHeldPrompts(Database database, HeldPromptRows rows) {
    ...
    @Override
    public Optional<String> take(Path stateDirectory, Role role, String sessionId) {
        return database.write(stateDirectory, () -> {
            ...
            return held.filter(row -> row.sessionId().equals(sessionId)).map(HeldPromptRow::prompt);
        });
    }
}
