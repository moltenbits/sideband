---
name: sideband
description: Sideband adapter for Codex. Join once per session so the Sideband discussion can push entries into this conversation with `codex queue`; record the human's prompts with `sideband append --from operator`; send with `sideband append`. No listener to run. Use when the user invokes $sideband or asks to talk to Claude through Sideband.
---
