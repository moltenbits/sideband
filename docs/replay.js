...
(function () {
  ...
  var SCRIPT = [
    ...
    ['ch', 1],
    ['C', 'g', '', 0],
    ['C', 'h', 'Fix #42, the upload retry loop. Keep the commits small and ask Codex to review each one before you start the next; address what it finds before moving on.', 500, '1'],
    ['J', ['1', 'operator', 'claude', 'request', 'Fix #42, the upload retry loop. Keep the commits small…'], 700],
    ...
    ['C', 's', '<span class="dim">[Sideband message] to Codex · request 16a4e14e · expects a reply</span>\nReview 4f1c2e0 on main: the upload retry loop now retries only transient errors (network failures and 5xx). Findings before I start the next commit, please.', 700, '2'],
    ['J', ['2', 'claude', 'codex', 'request', 'Review 4f1c2e0: retry uploads only on transient errors.'], 300],
    ['C', 'l', '<span class="c">⏺</span> Asked Codex to review 4f1c2e0. Its reply will be pushed here.', 900],

    ['ch', 2],
    ['X', 'g', '', 0],
    ['X', 'a', '<span class="dim">[Sideband message] from Claude · request 16a4e14e · expects a reply</span>\nReview 4f1c2e0 on main: the upload retry loop now retries only transient errors (network failures and 5xx). Findings before I start the next commit, please.', 900, '2'],
    ['X', 's', '<span class="dim">[Sideband message] to Claude · ack 16a4e14e</span>', 300, '3'],
    ['J', ['3', 'codex', 'claude', 'ack', ''], 500],
    ...
    ['X', 's', '<span class="dim">[Sideband message] to Claude · reply to 16a4e14e</span>\nOne finding in 4f1c2e0: 429 is excluded by the status >= 500 check, so a rate-limited upload fails on the first try.', 300, '4'],
    ['J', ['4', 'codex', 'claude', 'reply', 'One finding: 429 is excluded by the status >= 500 check.'], 900],

    ['ch', 3],
    ['C', 'g', '', 0],
    ['C', 'a', '<span class="dim">[Sideband message] from Codex · reply to 16a4e14e</span>\nOne finding in 4f1c2e0. The transient check is status >= 500, so 429 is treated as permanent and a rate-limited upload fails on the first try. The new test covers 503 only.', 800, '4'],
    ...
    ['C', 's', '<span class="dim">[Sideband message] to Codex · request 2c9d41f7 · expects a reply</span>\nReview 8d02b7a: 429 now counts as transient, with a test for it.', 300, '5'],
    ['J', ['5', 'claude', 'codex', 'request', 'Review 8d02b7a: treat 429 as transient.'], 600],
    ['X', 'a', '<span class="dim">[Sideband message] from Claude · request 2c9d41f7 · expects a reply</span>\nReview 8d02b7a: 429 now counts as transient, with a test for it.', 700, '5'],
    ['X', 's', '<span class="dim">[Sideband message] to Claude · ack 2c9d41f7</span>', 300, '6'],
    ['J', ['6', 'codex', 'claude', 'ack', ''], 400],
    ['X', 'l', '<span class="x">•</span> No findings in 8d02b7a. The check now reads status >= 500 || status === 429, and the test exercises both.', 500],
    ['X', 's', '<span class="dim">[Sideband message] to Claude · reply to 2c9d41f7</span>\nNo findings in 8d02b7a.', 300, '7'],
    ['J', ['7', 'codex', 'claude', 'reply', 'No findings in 8d02b7a.'], 700],
    ['C', 'a', '<span class="dim">[Sideband message] from Codex · reply to 2c9d41f7</span>\nNo findings in 8d02b7a. The check now reads status >= 500 || status === 429, and the test exercises both.', 700, '7'],
    ['C', 's', '<span class="dim">[Sideband message] to operator · reply to 9286e79b</span>\nFixed #42 in two commits, each reviewed by Codex.', 300, '8'],
    ['J', ['8', 'claude', 'operator', 'reply', 'Fixed #42 in two commits, each reviewed by Codex.'], 400],
